...
const getStatusBadge = (status: string) => {
  switch (status) {
    ...
    default:
      return <span className="bg-gray-200 text-gray-600 text-[9px] font-bold uppercase px-2 py-1 rounded">{status}</span>;
  }
};
          ...
          <div className="flex flex-col md:flex-row justify-between items-start">
            <div className="max-w-xl">
              ...
              <p className="mt-6 text-gray-500 leading-relaxed">
                Invite two client emails, save the invite record with album and photographer ownership, and send the invitation mail immediately.
              </p>
            </div>
            <button
        ...
        <main className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
          <div className="lg:col-span-1">
            <div className="bg-white p-6 rounded-2xl shadow-sm">
              ...
              <p className="text-sm text-gray-500 mb-6">Invite up to two emails and save the invite to the database.</p>

              <form onSubmit={handleSendInvitation}>
                <div className="mb-4">
                  <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider" htmlFor="email1">Primary Email</label>
                  <input
                    ref={emailInputRef}
                    type="email"
                    id="email1"
                ...
                <div className="mb-4">
                  <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider" htmlFor="email2">Secondary Email</label>
                  <input
                ...
                <div className="mb-6">
                  ...
                  <select
                    ...
                    style={{ color: '#211a1b' }}
                  >
                    {albums.length === 0 ? <option value="">No albums found</option> : null}
                    {albums.map((album) => (
              ...
              {message ? <p className="text-xs text-[#b10e6b] mt-4 text-center leading-relaxed">{message}</p> : null}
              <p className="text-xs text-gray-400 mt-4 text-center leading-relaxed">
                Collaborators receive an invite email and the invite is saved with album, photographer, and payment status.
              </p>
            </div>
          </div>
          ...
          <div className="lg:col-span-2">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-[#211a1b]">Saved Invite Records</h2>
            ...
            <div className="space-y-4">
              {invites.map((invite) => (
                <div key={invite._id} className="bg-white p-4 rounded-2xl shadow-sm flex items-center gap-4 hover:shadow-md transition-shadow">
                  <img
                    src={typeof invite.albumId === 'string' ? 'https://images.unsplash.com/photo-1522673607200-164d1b6ce8d2?w=400&q=80' : invite.albumId.coverPhoto || 'https://images.unsplash.com/photo-1522673607200-164d1b6ce8d2?w=400&q=80'}
                    alt={getAlbumTitle(invite.albumId)}
                    className="w-20 h-20 rounded-lg object-cover"
                  />
                  <div style={{ flexGrow: 1 }}>
                    <h3 className="font-semibold text-[#211a1b]">{getAlbumTitle(invite.albumId)}</h3>
                    <p className="text-xs text-gray-400">Emails: {invite.clientEmails.join(', ')}</p>
            ...
            <div className="flex justify-end items-center mt-6">
              <p className="text-xs text-gray-400 mr-4">Viewing latest invite records</p>
